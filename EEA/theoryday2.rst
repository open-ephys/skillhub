...
Impedance can be represented as a vector defined in terms of magnitude and phase (θ). Therefore, impedance provides both magnitude and phase relationship between V and I over a broad frequency range (from 1 Hz to 10 kHz). Let’s examine the response of resistors and capacitors to an applied sinusoidal voltage (i.e., AC signal).

Resistors
^^^^^^^^^^
For resistors, the impedance magnitude (Z) is constant and does not vary with the frequency of signal applied.
...
We saw in our equivalent circuits, that currents can flow to ground through the electrode circuit and the shunt capacitance (Cs), causing a difference between Vec and Vin.
...
.. math::
  ...
  |Vin| = Z|Cs|/(Z|Cs|+|Ze| ) |Vec|

Therefore, the crucial thing that determines how much of our voltage at the electrode tip makes it to our final recording, is the ratio of impedance between the electrode and the recording system. To get more of our voltage |Ve| into our recording system, we can:

1.	Reduce the electrode impedance |Ze|
2.	Increase the impedance |Zcs|
...
Electrode impedance magnitude is often measured at 1 kHz, before and after electrode coating, showing an impedance decrease up to 10-fold (Neto et al., 2018). By increasing the capacitance (|Ce|) of our electrode, the electrode impedance (|Ze|) will be smaller, preserving more of our signal amplitude at |Vin| (|Vin| = |Vec|).
One might still expect some attenuation of signal amplitude (|Vin| < |Vec|) because our electrodes impedance values are usually high (kOhm to MOhm at 1 kHz). However, electrodes are combined with recording systems with very high input impedance.
...
Amplifiers
***********
Let’s go over what an op-amp does. Make sure this all makes sense before moving on.
...
The op-amp has two inputs, and basically takes the voltage difference between them. If that difference is positive, it connects its output to one ‘power rail’, like for example 3V, and if it's negative it connects its output to the other rail, that could be ground, or -3V. Another way to say the same thing would be that it amplifies the difference between its input with a huge factor, also called ‘gain’.

While doing that, the op-amp draws basically no current on its inputs.
...
If we connect the output of the op-amp to the ‘-’ input, then the following happens:

-	Initially, the op-amp will output a very high voltage if the difference across it inputs is positive, and a very low one if its negative (like we said, the gain is extremely high).
...
We can look at the voltage that is on ‘+’ by just measuring the output of the op-amp, BUT because the ‘+’ input draws almost no current at all (in other words, very high input impedance), we can now measure weak signals. The output of the op-amp on the other hand side has very low output impedance, in other words we can draw a lot of current from it and it will keep its voltage.

Self-test: Now, if you haven't yet or want to do it again, run this example in the simulator, and see if what we said above about the op-amp makes sense. You should see that the op-amp stops the cable from drawing current from the electrode, and that the op-amp instead manages to ‘drive’ the cable effortlessly, by providing a lot of current.
...
To get from our neuronal currents to ground involves crossing the electrode impedance and then a path either through the amplifier to ground, or through shunting routes (|Cs|).
This shunt capacitance arises mainly from the capacitance across the thin insulation isolating an electrode shaft and the surrounding electrolyte, as well as the cumulative capacitance along cables and connector (Robinson, 1968). This route to ground, parallel to the amplifier, reduces the effective amplifier impedance, and being capacitive, this effect increases with signal frequency (Nelson et al., 2008). Therefore, the shunt capacitance should be small to create a large shunt impedance (:math: Z = \\frac{1}{2 \\pi fC}), especially when the electrodes have a large impedance, which are thus more susceptible to attenuation by shunt capacitance (Obien et al., 2015; Robinson, 1968). For instance, the shunt capacitance for a tungsten wire (~50 to 100 pF) is usually higher than for a silicon probe (5-20 pF/cm).

The effective amplifier input impedance, |Za|’ is the total impedance to the ground seen from the electrode, and it includes a path through the amplifier and shunting routes (shunt resistance and capacitance) to ground outside the amplifier.
To have no net current flowing, the shunt   and amplifier impedance should be infinite. Moreover, if |Za|’ is not substantially greater than |Ze|, |Vin| < |Vec|. Therefore, the best-case scenario is to have high |Za|’ and low |Ze|. To have low |Za|’, we need amplifiers with high input impedance and low shunt capacitance (high impedance).
Moreover, the output impedance of amplifiers is very low which enables driving the signal through all the subsequent circuits (e.g., interconnect lines, multiplexer, and ADC). By placing an amplifier in our circuit, we make sure that the rest of our recording circuit is driven by current provided by the amplifier, not the neurons.
